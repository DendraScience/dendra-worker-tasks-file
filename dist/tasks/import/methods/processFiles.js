...
async function processFiles(req, ctx) {
  ...
  try {
    runRes = await importer.run(req, ctx, spec, file => {
      ...
      return fs.unlink(file.path).then(() => fs.rmdir(tempPath)).catch(err => logger.warn('Temp rmdir error', {
        err,
        tempPath
      }));
    });
  ...
  result.processed = result.processed ? result.processed.concat(runRes.processed) : runRes.processed;
  /*
  ...
  if (runRes.files.length) {
    ...
    logger.info('Dispatching import', {
      importId
    });
